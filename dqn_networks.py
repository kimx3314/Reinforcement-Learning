...
class networks:
    ...
    def bellman(self, cur_reward, next_state):
        ...
        return cur_reward + (self.gamma * np.amax(self.model.predict(next_state)))
    ...
    def train(self, batch_size):
        ...
        for state, action, reward, next_state, done in replay_batch:
            # if the episode is finished, total return is given
            total_return = reward

            # if the episode is not finished, calculate the bellman total return
            if not done:
